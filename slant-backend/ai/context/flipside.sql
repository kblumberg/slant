...
with t0 as (
    ...
    where block_timestamp >= current_date() - interval '5 day'
    and succeeded
)
, t1 as (
    SELECT
    CAST(DATE_TRUNC('day', s.block_timestamp) AS DATE) AS date_time,
    case when t0.tx_id is not null then 1 else 0 end as is_jupiter_swap,
    SUM(
        CASE
        WHEN s.swap_from_mint IN (
            'So11111111111111111111111111111111111111112',
            'EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v',
            'Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB'
        )
        THEN s.swap_from_amount * pf.price
        WHEN s.swap_to_mint IN (
            'So11111111111111111111111111111111111111112',
            'EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v',
            'Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB'
        )
        THEN s.swap_to_amount * pt.price
        ELSE COALESCE(s.swap_from_amount * pf.price, s.swap_to_amount * pt.price)
        END
    ) AS total_usd
    FROM
    solana.defi.ez_dex_swaps s
    LEFT JOIN solana.price.ez_prices_hourly pf
        ON DATE_TRUNC('hour', s.block_timestamp) = pf.hour
        AND pf.blockchain = 'solana'
        AND pf.token_address = s.swap_from_mint
        AND pf.hour >= '2025-05-12'
        AND pf.hour < DATEADD(day, 1, CURRENT_DATE())
    LEFT JOIN solana.price.ez_prices_hourly pt
        ON DATE_TRUNC('hour', s.block_timestamp) = pt.hour
        AND pt.blockchain = 'solana'
        AND pt.token_address = s.swap_to_mint
        AND pt.hour >= '2025-05-12'
        AND pt.hour < DATEADD(day, 1, CURRENT_DATE())
    left join t0
        on t0.block_timestamp = s.block_timestamp
        and t0.tx_id = s.tx_id
    WHERE
    s.block_timestamp >= '2025-05-12'
    AND s.block_timestamp < DATEADD(day, 1, CURRENT_DATE())
...
with t0 as (
    ...
    FROM
    solana.defi.fact_swaps_jupiter_summary s
    LEFT JOIN solana.price.ez_prices_hourly pf
        ON DATE_TRUNC('hour', s.block_timestamp) = pf.hour
        AND pf.blockchain = 'solana'
        AND pf.token_address = s.swap_from_mint
    ...
    left join t0
        ...
        and t0.tx_id = s.tx_id
    WHERE
    s.block_timestamp::date = '2025-05-12'
    AND s.succeeded = TRUE
    GROUP BY
    1
)
...
To get the "% of XXX that is YYY" (where YYY is a subset of XXX), one method to consider is to select all the `tx_id`s from YYY and LEFT JOIN it to XXX and calculate it that way
...
, t1 as (
    select tx_id
    , block_timestamp
    , case when YYY.tx_id is not null then 1 else 0 end as is_included
    from XXX
    left join YYY
        on YYY.block_timestamp = XXX.block_timestamp
        and YYY.tx_id = XXX.tx_id
)
select * from t1;
```