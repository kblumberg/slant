...
def rag_search_queries(query: str, project_tags: list[str], top_k: int = 40, n_queries: int = 10) -> pd.DataFrame:
    ...
    results['mult'] = results['days_ago'].apply(lambda x: pow(0.9995, x)) * results['dashboard_id'].apply(lambda x: 1 if x else 0.9) * results['project_tags'].apply(lambda x: 1 if len(set(x) & set(project_tags)) else 0.95)
    results['score'] = results['original_score'] * results['mult']
    results['rk'] = results.groupby('user_id')['score'].rank(ascending=0, method='first')
    results['mult'] = results.rk.apply(lambda x: 0.985 ** x)
