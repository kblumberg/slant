...
from utils.flipside import extract_project_tags_from_query, extract_token_references_from_string
from utils.utils import clean_token_name
...
def update_tokens():
    ...
    query = "select id, text from flipside_queries where text ilike '%where%mint%' or text ilike '%where%token%'  "
    df = pg_load_data(query)

    tot = len(df)
    df['tokens'] = None
    df = df.reset_index(drop=True)
    for i, row in df[df.tokens.isnull()].iterrows():
        print(f'{i}/{tot}')
        if df.loc[i]['tokens'] is None:
            df.loc[i, 'tokens'] = extract_token_references_from_string(row['text'], llm)
    print(df.text.values[1])
    df.tokens
    df['tokens'] = df['tokens'].apply(lambda x: [ clean_token_name(tag) for tag in x ])
